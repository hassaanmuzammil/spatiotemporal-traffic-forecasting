import os
import torch
import torch.nn as nn
from torch_geometric.loader import DataLoader

from src.data.pyg_dataset import build_dataset
from src.models.gcn_baseline import GCNBaseline
from src.models.gcn_attention import GCNSpatioTemporalAttention
from src.models.stgcn import SpatioTemporalGNN
from src.models.gcn_rnn import GCNTemporalRNN

from src.train.train import train
from src.train.evaluate import test
from src.train.utils import plot_losses, create_run_dir
import src.configs.config as cfg


def main():
    # ── mlflow setup ──
    mlflow_run = None
    if cfg.ENABLE_MLFLOW:
        import mlflow

        mlflow.set_tracking_uri(cfg.MLFLOW_TRACKING_URI)
        mlflow.set_experiment(cfg.MLFLOW_EXPERIMENT_NAME)
        mlflow_run = mlflow.start_run()

        config_params = {
            k: str(v)
            for k, v in vars(cfg).items()
            if not k.startswith("_") and not isinstance(v, (dict, type(__builtins__)))
        }
        mlflow.log_params(config_params)

    # ── load dataset ──
    train_ds, val_ds, test_ds, mean, std = build_dataset("METR-LA")

    train_loader = DataLoader(train_ds, batch_size=cfg.batch_size, shuffle=False)
    val_loader = DataLoader(val_ds, batch_size=cfg.batch_size, shuffle=False)
    test_loader = DataLoader(test_ds, batch_size=cfg.batch_size, shuffle=False)

    # ── log dataset info ──
    if mlflow_run is not None:
        import mlflow

        mlflow.log_params({
            "dataset": cfg.dataset,
            "train_samples": len(train_ds),
            "val_samples": len(val_ds),
            "test_samples": len(test_ds),
            "data_mean": round(float(mean), 6),
            "data_std": round(float(std), 6),
        })

    # ── model ──

    # Baseline GCN
    # model = GCNBaseline(
    #     cfg.in_feats,
    #     cfg.hidden_feats,
    #     cfg.out_feats,
    # ).to(cfg.device)

    # STGCN
    model = SpatioTemporalGNN(
        cfg.in_feats,
        cfg.hidden_feats,
        cfg.out_feats,
        cfg.num_layers,
        cfg.num_heads,
        cfg.dropout
    ).to(cfg.device)

    # GCN Spatiotemporal Attention
    # model = GCNSpatioTemporalAttention(
    #     in_feats=cfg.in_feats,
    #     temporal_hidden=cfg.temporal_feats,
    #     hidden_feats=cfg.hidden_feats,
    #     out_feats=cfg.out_feats,
    #     num_heads=cfg.num_heads,
    #     window_size=cfg.window_size,
    # ).to(cfg.device)

    # GCN + RNN
    # model = GCNTemporalRNN(
    #     seq_len=cfg.window_size,
    #     temporal_hidden=cfg.temporal_feats,
    #     hidden_feats=cfg.hidden_feats,
    #     out_feats=cfg.out_feats,
    #     rnn_type="gru",
    #     input_dim=cfg.in_feats,
    # ).to(cfg.device)

    # ── optimizer / loss / scheduler ──
    optimizer = torch.optim.Adam(model.parameters(), lr=cfg.lr, weight_decay=1e-4)
    criterion = nn.HuberLoss(delta=1.0)
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer,
        patience=5,
        factor=0.5
    )

    num_params = sum(p.numel() for p in model.parameters())
    print(f"Model parameters: {num_params:,}")

    if mlflow_run is not None:
        import mlflow
        mlflow.log_param("model_parameters", num_params)

    # ── create checkpoint folder ──
    run_ckpt_dir = create_run_dir()
    print(f"Checkpoints will be saved to -> {run_ckpt_dir}")

    # ── train ──
    train_losses, val_losses = train(
        model=model,
        train_loader=train_loader,
        val_loader=val_loader,
        optimizer=optimizer,
        criterion=criterion,
        device=cfg.device,
        epochs=cfg.epochs,
        ckpt_dir=run_ckpt_dir,
        print_every=cfg.print_every,
        save_every=cfg.save_every,
        scheduler=scheduler,
        mlflow_run=mlflow_run,
    )

    plot_losses(train_losses, val_losses, run_ckpt_dir)

    # ── test ──
    print("\nTesting best model...")
    best_model_path = os.path.join(run_ckpt_dir, "best_model.pt")
    model.load_state_dict(torch.load(best_model_path, map_location=cfg.device))

    test_metrics = test(model, test_loader, mean, std, cfg.device)

    if isinstance(test_metrics, dict):
        print(f"rmse: {test_metrics.get('rmse', float('nan')):.4f}")
        print(f"mse: {test_metrics.get('mse', float('nan')):.4f}")
        print(f"mae: {test_metrics.get('mae', float('nan')):.4f}")
    else:
        print("Test metrics:", test_metrics)

    # ── log test metrics ──
    if mlflow_run is not None and isinstance(test_metrics, dict):
        import mlflow
        mlflow.log_metrics({
            "test_rmse": test_metrics.get("rmse", float("nan")),
            "test_mse": test_metrics.get("mse", float("nan")),
            "test_mae": test_metrics.get("mae", float("nan")),
        })

    # ── end mlflow run ──
    if mlflow_run is not None:
        import mlflow
        mlflow.end_run()


if __name__ == "__main__":
    torch.manual_seed(42)
    main()