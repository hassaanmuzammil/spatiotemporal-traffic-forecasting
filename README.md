...
> A systematic bottom-up study of spatio-temporal GNN architectures for short-term traffic speed forecasting on METR-LA and PEMS-BAY.

---

## Overview

Urban traffic congestion propagates across interconnected road networks in ways that purely temporal models cannot capture. Rather than proposing a single complex model, this project takes an **incremental architectural approach** — empirically validating each design decision before inclusion and providing clear evidence for why spatio-temporal GNNs work.

**Key finding:** Replacing static GCN aggregation with attention-weighted GATConv yields a **37% MAE reduction**, representing the single largest architectural gain in the study.
...
Our best model outperforms Graph WaveNet on both datasets. The remaining gap to T-Graphormer is attributable to its significantly larger architecture and training budget.
...
The best model follows a **temporal-first spatio-temporal GNN** pipeline:
...
The temporal-first ordering is deliberate, as spatial aggregation before temporal encoding allows raw neighbor noise to corrupt temporal representations.

### Training

- **Loss:** Huber loss (δ = 1.0) for robustness to outliers
...
- **Batch size:** 64, trained up to 50 epochs
- **Experiment tracking:** MLflow (hyperparameters, training curves, checkpoints)

### Data Preprocessing

- Missing values handled using bidirectional linear interpolation
- Chronological train/validation/test split (70:10:20) to avoid leakage
- Z-score normalization using train-set statistics only
- ~24,000 training samples for METR-LA and ~36,000 for PEMS-BAY
...
Place the `datasets` folder in the project root directory.
...
All dependencies and exact package versions are locked in `uv.lock` for reproducible environments across platforms.

---

## Training

Set the configuration in `src/configs/config.py` and run:
...
Adjust best model path and architecture, then run:
```
...
1. **Temporal patterns dominate spatial structure in isolation.**  
   LSTM alone (MAE 4.55) significantly outperforms GCN alone (MAE 6.01), indicating that rush-hour and weekly traffic cycles provide stronger predictive signal than topology without temporal context.

2. **GATv2 attention is the most impactful architectural component.**  
   Replacing GCN with GATv2 reduces MAE from 5.45 → 3.42 (37% improvement). Learned asymmetric attention captures which neighboring sensors are most relevant for forecasting.

3. **Transformers provide limited gains for short sequences.**  
   Full self-attention over only 12 time steps is computationally expensive and provides minimal improvement over the simpler LSTM + GATv2 configuration.

4. **Explicit feature engineering offers marginal benefit.**  
   Calendar and centrality features contribute negligible improvement, suggesting that attention mechanisms already capture much of the underlying temporal and structural context.
